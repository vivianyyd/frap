...
(* Welcome to the wonderful world of Rocq programming! *)

(* This pset dives straight in to building a program-analysis pass and
 * proving it sound in Rocq. But before that, you will need to get set up
 * with Rocq. *)
 ...
 *
 * First, install opam using these instructions: <https://opam.ocaml.org/doc/Install.html>
 * Make sure opam version is 2.1.0 or above.
 *
 * $ opam --version
 *
 * Initialize opam before the first usage:
 *
 * $ opam init
 * $ eval $(opam env)
 *
 ...
 *
 * Run `rocq -v` to check that the version 9.0.0 was installed.
 *
 ...
 * given in the extension's overview page, as you will get an old-dated Rocq version if you do!
 * Just run this command instead:
 *
 * $ opam install vscoq-language-server
 *
 * After the commands above finishes, run `vscoqtop -v` to make sure you have the right version installed.
 * Lastly, run `which vscoqtop` to get its path and put it in the VsCode settings
 * under `Vscoq: Path`.
 * Make sure you open your VsCode window in the <pset01_ProgramAnalysis> directory.
 * Otherwise the extension cannot properly pick up and understand the <_CoqProject> file.
 *
 * We will have special office hours the first week of class,
 * to help everyone get these packages set up.
 *
 * Now, on to the actual assignment, once you have Rocq and a UI installed!
 * This course uses a special Rocq library, so we'll start by compiling that:
 *
 ...
 * Rocq or structuring proofs, we recommend reading the tips section found near the
...
Module Type S.
   ...
   * by the Rocq standard library, and for this part of the
  ...
  (* such that if this function returns [true], the program would not trigger
     ...
     division by zero, but it must recognize as good the examples given below.
  ...
  (* Before diving into the Rocq proof, try to convince yourself that your code
...
- For help on standard Rocq tactics, consult Rocq's reference manual (https://rocq-prover.org/doc/master/refman/index.html), starting from the indices at https://rocq-prover.org/doc/master/refman/appendix/indexes/index.html.  The manual can be overwhelming, so it's best-used for looking up fine details.

Useful commands
---------------

Rocq comes with many predefined types, functions, and theorems (“objects”).  The most important commands to help you discover them are `Check`, `About`, `Print`, `Search`, and `Compute`.  Try the following examples:
...
(* Find theorems about "+". *)
Search "+".
(* Find theorems whose statements mention S and eq. *)
Search eq S.
...
(*|
In Rocq, `if` is just short for `match`:
|*)
