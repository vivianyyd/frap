...
Module Swap.
  ...
  Proof.
    unfold swap'.
    intros.
    repeat (rewrite ?length_app, ?length_firstn, ?length_skipn; cbn [length]).
  ...
  Proof.
    ...
    destruct (Nat.leb_spec (length (l ++ l')) n2);
      simplify; rewrite length_app in *; try linear_arithmetic.
...
Since we match on `partial_eval c` in some cases, you will need to know that the output of
`partial c` is well-typed. Fortunately we have a lemma for this: `partial_eval_sound`.
Expect to use this lemma in the proof of `partial_eval_correct` in the cases where we match
on `partial_eval c`.
...
about `val_flatmap` in a setting without typing assumptions.
