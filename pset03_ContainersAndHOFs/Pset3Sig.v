...
(* Here's a technical note on why this pset overrides a Frap tactic.
   ...
   and there's a Hint in Stdlib.Program.Combinators
...
"~ P" in Rocq stands for "not P", and is defined as "P -> False", so to prove "~ P", it sometimes helps to do "unfold not. simplify."
...
(*|
Note that the syntax [{struct x}] is a hint to Rocq that our definition structurally recurses on the [x] argument (the only argument). Of course, there isn't any recursion at all here! So indeed, every recursive call (of which there are none) passes a structurally smaller [x] argument to [always_true].
...
Here's a key fact to understanding how [simplify] works on [Fixpoint] definitions: [simplify] only reduces the application of [Fixpoint] definition when the argument on which it structurally recurses (according to Rocq) is in the form of a constructor application.
...
Finally, since Rocq usually infers which argument a fixpoint is structurally decreasing on, you can run [Print f] for any fixpoint [f] to see which argument Rocq chose.
|*)
