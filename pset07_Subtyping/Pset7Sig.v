...
To discuss this case, we will use `A`s for argument types and `B`s for return types, and use numbers such that they match the subtyping order, e.g. `A0 $<: A1 $<: A2 $<: A3` etc. Rocq's autogenerated names will look different, and we encourage you to use commands like `rename t1 into myNewName` to give more intuitive names to the variables.
